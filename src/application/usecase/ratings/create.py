...
class RatingCreateUseCase:
    ...
    async def execute(
        self, movie_id: int, tg_user_id: int, rating: int
    ) -> Rating | None:
        try:
            user: User | None = await self.user_repository.get(
                tg_user_id, field_search="tg_user_id"
            )
            movie: Movie | None = await self.movie_repository.get(
                movie_id, field_search="id"
            )
            ...
            return await self.rating_repository.add(
                Rating(user=user, movie=movie, rating=rating, timestamp=None)
            )
